...
const messagingApiBaseUrl =
  ...
  'https://southamerica-east1-coloniadeferiasss.cloudfunctions.net/paymentApi';

const evolutionProxyUrl =
  (import.meta.env.VITE_WORKER_URL as string) ||
  'https://arenasimonesia-whatsapp-proxy.thayrufino2.workers.dev';

export const ARENA_INSTANCE_PREFIX = 'arena_simonesia_';
...
const settingsRef = doc(db, 'system_settings', 'arena_evolution_messaging');
...
export const toArenaInstanceName = (value: string) => {
  const normalized = normalizeInstanceName(value.replace(new RegExp(`^${ARENA_INSTANCE_PREFIX}`), ''));
  return normalized.startsWith(ARENA_INSTANCE_PREFIX)
    ? normalized
    : `${ARENA_INSTANCE_PREFIX}${normalized}`;
};

const assertArenaInstance = (instanceName: string) => {
  if (!instanceName.startsWith(ARENA_INSTANCE_PREFIX)) {
    throw new Error('Esta instância não pertence ao sistema da Arena Simonésia.');
  }
};
...
export const getMessagingSettings = async (): Promise<MessagingSettings> => {
  ...
  return {
    ...emptySettings,
    ...data,
    instanceName: instanceName.startsWith(ARENA_INSTANCE_PREFIX) ? instanceName : ''
  };
};

export const saveMessagingSettings = async (settings: MessagingSettings) => {
  if (settings.instanceName) assertArenaInstance(settings.instanceName);
...
export const getEvolutionInstances = async () => {
  ...
  return data.instances.filter((instance) => instance.name.startsWith(ARENA_INSTANCE_PREFIX));
};

export const createEvolutionInstance = async (rawInstanceName: string) => {
  const instanceName = toArenaInstanceName(rawInstanceName);
  assertArenaInstance(instanceName);
...
export const testEvolutionMessage = async (instanceName: string, phone: string) => {
  ...
  await sendEvolutionText(instanceName, phone, [
    ...
    `*Data/hora:* ${now}`,
    '',
    'Resultado esperado: se esta mensagem chegou, a instância da Arena está conectada e apta para receber os gatilhos automáticos.'
  ].join('\n'));
};
